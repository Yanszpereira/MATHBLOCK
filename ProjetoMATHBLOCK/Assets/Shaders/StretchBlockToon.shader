...
fixed4 frag(V i):SV_Target{fixed4 a=tex2D(_MainTex,i.uv)*_BaseColor;float3 n=normalize(i.n),l=normalize(UnityWorldSpaceLightDir(i.wp)),viewDir=normalize(UnityWorldSpaceViewDir(i.wp));UNITY_LIGHT_ATTENUATION(att,i,i.wp);float x=saturate(dot(n,l))*att;float s=max(2,round(_ShadeSteps)),z=x*(s-1),aa=max(fwidth(z)*_ShadeSmoothness,.0001);float q=saturate((floor(z)+smoothstep(1-aa,1,frac(z)))/(s-1));float3 c=a.rgb*(lerp(_ShadeColor.rgb*_MinBrightness,_MaxBrightness.xxx,q)*_LightColor0.rgb+ShadeSH9(float4(n,1)).rgb*_AmbientStrength);
#ifdef _SPECULAR_ON
float sp=pow(saturate(dot(n,normalize(l+viewDir))),lerp(8,128,_Glossiness)),wa=max(fwidth(sp),.0001);c+=_SpecularColor.rgb*smoothstep(.5-wa,.5+wa,sp)*att;
#endif
#ifdef _RIM_ON
float r=1-saturate(dot(n,viewDir)),wr=max(fwidth(r),.0001);c+=_RimColor.rgb*smoothstep(_RimAmount-wr,_RimAmount+wr,r)*q;
#endif
