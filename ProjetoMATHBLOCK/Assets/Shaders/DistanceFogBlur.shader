    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f_img input) : SV_Target
            {
                ...
                fixed4 blurred = center * 0.32;
                blurred += tex2D(_MainTex, input.uv + float2(offset.x, 0)) * 0.13;
                blurred += tex2D(_MainTex, input.uv - float2(offset.x, 0)) * 0.13;
                blurred += tex2D(_MainTex, input.uv + float2(0, offset.y)) * 0.13;
                blurred += tex2D(_MainTex, input.uv - float2(0, offset.y)) * 0.13;

                // Diagonais são removidas no Android para reduzir leituras de textura.
                if (_MobileQuality < 0.5)
                {
                    blurred += tex2D(_MainTex, input.uv + offset) * 0.04;
                    blurred += tex2D(_MainTex, input.uv - offset) * 0.04;
                    blurred += tex2D(_MainTex, input.uv + float2(offset.x, -offset.y)) * 0.04;
                    blurred += tex2D(_MainTex, input.uv + float2(-offset.x, offset.y)) * 0.04;
                }
                else
                {
                ...
                fixed3 color = lerp(center.rgb, blurred.rgb, fog);
                ...
                float dot = 1.0 - smoothstep(0.18, 0.29, length(cell));
